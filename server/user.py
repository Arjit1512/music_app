...
async def lifespan(app: FastAPI):
    # Startup code: Connect to database
    try:
        app.mongodb_client = AsyncIOMotorClient(MONGO_URL)
        app.mongodb = app.mongodb_client[DB_NAME]
        app.collection = app.mongodb["users"]
        app.reviews = app.mongodb["reviews"]
        print("Successfully connected to the MongoDB database")
    except Exception as e:
        print(f"Failed to connect to the database: {e}")
    
    yield  # This is where FastAPI runs and handles requests
    
    # Shutdown code: Close database connection
